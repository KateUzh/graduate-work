...
Спроектированы DTO и модели данных (JPA Entities).
Реализован слой контроллеров (REST API) согласно спецификации OpenAPI.
Реализован персистентный слой и миграции базы данных (Liquibase).
Внедрена ролевая модель доступа (Spring Security + BCrypt).
Реализована бизнес-логика сервисов с низким уровнем связности (Low Coupling).
Достигнуто целевое покрытие тестами (JUnit 5, Mockito, MockMvc).
...
config: Инфраструктурные настройки (раздача статических ресурсов, поддержка Basic Auth и Multipart-запросов в Swagger).
security: Комплексная безопасность: инфраструктурная настройка (Basic Auth, CORS, BCrypt) и кастомная проверка прав доступа (User/Author/Admin) на уровне сервисов.
mappers: Преобразование данных между слоями (Entity - DTO).
exceptions: Централизованная обработка ошибок (@RestControllerAdvice).
constants: Централизованное управление сообщениями об ошибках для единообразных ответов API.
Обзор основного API
...
Отредактируйте параметры подключения в файле src/main/resources/application.yaml: Укажите ваши username и password от локального PostgreSQL. Проверьте путь к хранилищу изображений (параметр app.upload.main-dir). Убедитесь, что у приложения есть права на запись в эту папку.
...
Документация API
После запуска интерактивная документация Swagger UI доступна по адресу: http://localhost:8080/swagger-ui/index.html
